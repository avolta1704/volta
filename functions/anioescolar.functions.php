...
class FunctionAnioEscolar
{
  ...
  public static function getButtonsGradoCerrarAnioEscolar($idGrado, $status)
  {
    // Evaluar el estado para determinar si el botón de asignar alumnos debe estar deshabilitado
    ...
    $botones = '
      <div class="btn-group">
        ...
        <ul class="dropdown-menu" aria-labelledby="dropDownCerrarAnioEscolarGrados">
          <div>
            <button type="button" class="dropdown-item btnAsignarAlumnosNuevoAnio" idGrado="' . ($idGrado) . '" data-bs-toggle="modal" data-bs-target="#modalCerrarAnioAlumnos" ' . $disabled . '>Alumnos</button>
          </div>
        </ul>
      </div>
      ';
