...
$("#hermanosMatriculadosSwitch").change(function () {
  if ($(this).is(":checked")) {
    var data = new FormData();
    data.append("todosLosAlumnosMatriculadosApoderados", true);
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        // Obtén el select existente
        var select = $("#hermanosMatriculadosSelect");
        // Agrega las opciones al select
        select.append(opciones);
      },
      error: function (jqXHR, textStatus, errorThrown) {
...
// Detectar cuando se selecciona una opción en el select
$("#hermanosMatriculadosSelect").change(function () {
  // Obtener la opción seleccionada
  var opcionSeleccionada = $(this).find("option:selected");

  // Obtener los atributos data-* de la opción seleccionada
  var idPadre = opcionSeleccionada.data("padre");
  var idMadre = opcionSeleccionada.data("madre");
  obtenerDatosApoderadoPostulante(idPadre)
    .then((response) => {
      // Asignar los valores a los inputs del formulario
      document.getElementById("nombrePadre").value =
        response["nombreApoderado"];
      document.getElementById("apellidoPadre").value =
        response.apellidoApoderado;
      document.getElementById("dniPadre").value = response["dniApoderado"];
      document.getElementById("fechaNacimientoPadre").value =
        response["fechaNacimiento"];
      document.getElementById("gradoPadre").value =
        response["gradoInstruccion"];
      document.getElementById("profesionPadre").value =
        response["profesionApoderado"];
      document.getElementById("emailPadre").value = response["correoApoderado"];
      document.getElementById("numeroPadre").value =
        response["celularApoderado"];
      document.getElementById("dependenciaPadre").value =
        response["dependenciaApoderado"];
      document.getElementById("centroPadre").value = response["centroLaboral"];
      document.getElementById("numeroTrabajoPadre").value =
        response["telefonoTrabajo"];
      document.getElementById("ingresoPadre").value =
        response["ingresoMensual"];

      // Asignar el valor del radio button
      if (response["convivenciaAlumno"] === "Si") {
        document.getElementById("cbxSiConvive").checked = true;
      } else {
        document.getElementById("cbxNoConvive").checked = true;
      }
    })
    .catch((error) => {
  ...
  obtenerDatosApoderadoPostulante(idMadre)
    .then((response) => {
      // Asignar los valores a los inputs del formulario de la madre
      document.getElementById("nombreMadre").value =
        response["nombreApoderado"];
      document.getElementById("apellidoMadre").value =
        response["apellidoApoderado"];
      document.getElementById("dniMadre").value = response["dniApoderado"];
      document.getElementById("fechaNacimientoMadre").value =
        response["fechaNacimiento"];
      document.getElementById("gradoMadre").value =
        response["gradoInstruccion"];
      document.getElementById("profesionMadre").value =
        response["profesionApoderado"];
      document.getElementById("emailMadre").value = response["correoApoderado"];
      document.getElementById("numeroMadre").value =
        response["celularApoderado"];
      document.getElementById("dependenciaMadre").value =
        response["dependenciaApoderado"];
      document.getElementById("centroMadre").value = response["centroLaboral"];
      document.getElementById("numeroTrabajoMadre").value =
        response["telefonoTrabajo"];
      document.getElementById("ingresoMadre").value =
        response["ingresoMensual"];
...
function obtenerDatosApoderadoPostulante($idApoderado) {
  return new Promise((resolve, reject) => {
    var data = new FormData();
    data.append("idApoderadoPostulanteHermano", $idApoderado);
    ...
    $.ajax({
      ...
      dataType: "json",
      success: function (response) {
        // Resolver la promesa con el response
        resolve(response);
      },
      error: function (jqXHR, textStatus, errorThrown) {
        console.log(jqXHR.responseText);
